    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Right Side Single Image (Now with Transitions & Parallax Hover) */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.95, x: 20 }}
            animate={isInView ? { opacity: 1, scale: 1, x: 0 } : {}}
            transition={{ duration: 0.8, delay: 0.3, ease: "easeOut" }}
            className="relative w-full max-w-4xl mx-auto lg:ml-auto"
          >
            {/* Breathing Animation Wrapper */}
            <motion.div 
              animate={{ y: [-8, 8, -8] }}
              transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
              className="relative h-[600px] rounded-2xl overflow-hidden shadow-2xl shadow-[#3B82F6]/15 border border-white/10 group cursor-pointer"
            >
              {/* Hover Scale Image */}
              <motion.img 
