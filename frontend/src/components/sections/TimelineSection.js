...
const TimelineSection = () => {
  ...
  // Safe layout detection for SSR/Client discrepancy
  const [isMobile, setIsMobile] = useState(
    typeof window !== "undefined" ? window.innerWidth < 768 : false
  );

  // Readjust constraints silently on orientation change / resize
  useEffect(() => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // DESKTOP: Complex HW accelerated horizontal pin
      ...
      // MOBILE: Lightweight, frictionless native vertical scrolling with elegant reveals
      mm.add("(max-width: 767px)", () => {
        ...
        // Simplified vertical parallax for background optimization
        gsap.to(".starfield", {
          yPercent: -20,
          ease: "none",
          scrollTrigger: {
    ...
    return () => ctx.revert();
  }, [isMobile]); // Reload context if bounds inherently change
              ...
              <div key={index} className="timeline-panel relative w-full px-6 md:px-12 md:w-[50vw] md:h-full flex items-center justify-center flex-shrink-0">
                ...
                <div className="timeline-card relative group z-10 w-full max-w-[550px]">
                  ...
                  >
                    ...
                    <h3 className="text-2xl md:text-4xl font-bold mb-4 md:mb-5 bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400 relative z-10">
                      {item.title}
                    </h3>
